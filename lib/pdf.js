...
export async function buildPdf(data) {
  const format = (data?.meta && data.meta.pageSize) ? String(data.meta.pageSize).toLowerCase() : 'a5';
  ...
  // Signatures (stick to ~18mm from bottom regardless of page size)
  const ySign = pageH - 18;
  ...
  // Footer (stick to ~8mm from bottom regardless of page size)
  doc.setFontSize(8);
