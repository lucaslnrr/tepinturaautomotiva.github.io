...
function renderSlip(doc, data, box, logoImg, isBottom) {
  ...
  if (false && contactsLine) {
    doc.text(contactsLine.replace(/\u0007/g, ' • '), x + w - innerPad, y + 16, { align: 'right' });
  }

  // Badge tipo + número
  // Fix header right overlap and wrap contacts/address
  { const rightX = x + w - innerPad - 52 - 3;
    const fullContacts = [data.company?.address, data.company?.contacts].filter(Boolean).join(' • ');
    const wrapped = doc.splitTextToSize([data.company?.cnpj, fullContacts].filter(Boolean).join(' • '), 90);
    const clearX = x + w - innerPad - 86;
    doc.setFillColor(...BRAND.primary);
    doc.rect(clearX, y, 86, headerH, 'F');
    doc.setTextColor(255);
    doc.text(String(wrapped[0] || ''), rightX, y + 19, { align: 'right' });
    if (wrapped.length > 1) doc.text(String(wrapped[1] || ''), rightX, y + 23, { align: 'right' });
  }
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.5);
  // Vertically center within badge (badge top = y+4.5, center ~ y+10.5)
  doc.text(tipo, x + w - innerPad - badgeW + 3, y + 8.9);
  doc.setFontSize(11.5);
  doc.text(num, x + w - innerPad - 3, y + 11.0, { align: 'right' });
  ...
  let tableStartY = yCar + 22;
  // Adjust vehicle block dynamically to avoid overlap and update table start
  try {
    const rightReserve = 45;
    const leftSpanW = Math.max(60, w - innerPad * 2 - rightReserve);
    // Clear original interior
    doc.setFillColor(255,255,255);
    doc.rect(x + 0.4, yCar + 0.2, w - 0.8, 20.2, 'F');
    const v = data.vehicle || {};
    const vTitleFix = `Veiculo: ${v.marca || ''} / ${v.modelo || ''}   Ano: ${v.ano || ''}   Cor: ${v.cor || ''}`;
    const vLinesFix = doc.splitTextToSize(vTitleFix, leftSpanW);
    for (let i = 0; i < vLinesFix.length; i++) {
      doc.text(String(vLinesFix[i] || ''), x + innerPad, yCar + 6 + i * 4);
    }
    const yRow2Fix = yCar + 6 + vLinesFix.length * 4 + 2;
    doc.text(`Placa: ${v.placa || ''}`, x + innerPad, yRow2Fix);
    doc.text(`KM: ${v.km || ''}`, x + w - innerPad, yRow2Fix, { align: 'right' });
    const yRow3Fix = yRow2Fix + 6;
    const meta = data.meta || {};
    doc.text(`Pagamento: ${meta.pagamento || ''}`, x + innerPad, yRow3Fix);
    doc.text(`Prazo de entrega: ${meta.prazo || ''}`, x + w - innerPad, yRow3Fix, { align: 'right' });
    const vBoxHFix = Math.max(18, (yRow3Fix + 3) - yCar);
    doc.roundedRect(x, yCar, w, vBoxHFix, 2, 2);
    tableStartY = Math.max(tableStartY, yCar + vBoxHFix + 4);
  } catch(_){ }
  autoTable(doc, {
    startY: tableStartY,
