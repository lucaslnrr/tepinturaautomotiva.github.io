...
function parseBR(x) {
  ...
  if (!x) return 0;
  return Number(String(x).replace(/[data.company?.address, data.company?.contacts]/g, '').replace(',', '.')) || Number(x) || 0;
}
...
function renderSlip(doc, data, box, logoImg) {
  ...
  const leftColW = Math.min(40, Math.max(28, w * 0.16));
  ...
  const rightInfoRaw = [data.company?.cnpj, data.company?.address, data.company?.contacts]
    .filter(Boolean)
    .join(' • ')
    .replace(/\u0007/g, ' • ');
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.text(tipo, badgeX + 3, badgeY + 4.4);
  doc.setFontSize(11.5);
  doc.text(num, badgeX + badgeW - 3, badgeY + 6.5, { align: 'right' });
  ...
  const vLine1 = `Veículo: ${v.marca || ''} / ${v.modelo || ''}   Ano: ${v.ano || ''}   Cor: ${v.cor || ''}`;
  doc.setFontSize(9.5);
  ...
  doc.text(`Placa: ${v.placa || ''}`, x + innerPad, yRow2);
  doc.text(`KM: ${v.km || ''}`, x + w - innerPad, yRow2, { align: 'right' });

  const yRow3 = yRow2 + 6;
  doc.text(`Cond. Pagto: ${meta.pagamento || ''}`, x + innerPad, yRow3);
