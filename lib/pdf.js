...
function renderSlip(doc, data, box, logoImg) {
  ...
  const contentHeights = [
    ...
    // mid col (name + tag + CNPJ + endereço + Contato)
    6
      + nameLines.length * (lineH + 1)
      + tagLines.length * (lineH - 0.2)
      + (cnpjText ? (lineH - 0.2) : 0)
      + (addrLines.length ? addrLines.length * (lineH - 0.2) : 0)
      + (addrLines.length ? (lineH - 0.2) : 0) // espaço após endereço
      + (addrLines.length ? (lineH - 0.2) : 0) // linha "Contato"
      + 6,
    // right col (info)
  ...
  if (addrLines && addrLines.length) {
    ...
    // Linha solicitada: Contato abaixo do endereço
    doc.text('Contato: 27 9.9876-2687', midX, midY, { align: 'left' });
    midY += lineH - 0.2;
  }
  ...
  doc.text(`Forma de Pagamento: ${pagamentoU}`, x + innerPad, yRow3);
...
'• Valores válidos conforme período informado.\n• Orçamento não configura OS sem aceite formal do cliente.\n• Serviços sujeitos à análise de peças e estado da pintura.';
